...
;
;	ORG	(MEM-7)*1024
DEFINE Z80CPM, SPACE = ROM, ORG	= CCP
	segment Z80CPM
	.assume	ADL=0
...
;
SAVE2:	LD	DE,_FCB		;now close the file.
	CALL	CLOSE
...
;
UNKWN1:	LD	DE,_FCB+9	;an extension specified?
	LD	A,(DE)
	CP	' '
...
GETBACK1: CALL	CONVFST		;convert first name in (_FCB).
...
;
	xref BOOT		;cold start
	xref WBOOT		;warm start
	xref CONST		;console status
	xref CONIN		;console character in
	xref CONOUT		;console character out
	xref LIST		;list character out
	xref PUNCH		;punch character out
	xref READER		;reader character out
	xref HOME		;move head to home position
	xref SELDSK		;select disk
	xref SETTRK		;set track number
	xref SETSEC		;set sector number
	xref SETDMA		;set dma address
	xref READ		;read disk
	xref WRITE		;write disk
	xref LISTST		;return list status
	xref SECTRN		;sector translate
endif ;CPM22	

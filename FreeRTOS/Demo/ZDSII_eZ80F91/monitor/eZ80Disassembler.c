...
typedef struct
{
  uint8_t *InstructionName;
  uint8_t *InstructionArguments;
  uint8_t *InstructionSuffix;
  uint8_t *StartPosition;
  uint8_t Length;
  uint8_t IsBranch;
  uint8_t *BranchTarget;
} DisassembledInstruction_t;
...
typedef enum
{
  ...
  Lis,
  Lil
} AddressingModePrefix_t;


static uint8_t *Data;
static uint8_t LongData;
static unsigned CurrentByte;


uint8_t *
ToString (const DisassembledInstruction_t * instr, uint8_t * str,
	  unsigned strsz, uint8_t indentArguments)
{
  if (!indentArguments)
    snprintf (str, strsz, "%s%s%s%s",
	      instr->InstructionName,
	      InstructionSuffix,
	      (!InstructionArguments
	       || !*InstructionArguments ? "" : " "), InstructionArguments);
  else
    snprintf (str, strsz, "%s%s%*s",
	      instr->InstructionName,
	      InstructionSuffix,
	      (!InstructionArguments
	       || !*InstructionArguments ? 1 : 10), InstructionArguments);
  return instr;
}
...
static uint8_t *
SignedByte (uint8_t * str, unsigned strsz, int x)
{
  if (x >= 0)
    snprintf (str, strsz, "%2.2X", x);
  else
    snprintf (str, strsz, "-%2.2X", -x);
  return str;
}
...
void
DisassembleInstruction (uint8_t str, unsigned strsz)
{
  int branchTarget;
	...
	case 0:
	  switch (GetField (Field.y, b))
		...
		CurrentInstruction.InstructionArguments =
		  LabelPrefixString +
		  branchTarget.
		  ToString (Z80PlainMode ? Format2Byte : Format3Byte);
	      else
		CurrentInstruction.InstructionArguments =
		  SignedByte (branchTarget);
	      break;
	    case 3:
	      branchTarget = (sbyte) Data[CurrentByte++];
	      if (HasBaseAddress)
		...
		branchTarget &= 0xFFFF;
	      ...
	      if (HasBaseAddress)
		CurrentInstruction.InstructionArguments =
		  TableCC[GetField (Field.qq, b)] + ", " + LabelPrefixString +
		  branchTarget.
		  ToString (Z80PlainMode ? Format2Byte : Format3Byte);
	      else
		CurrentInstruction.InstructionArguments =
		  TableCC[GetField (Field.qq, b)] + ", " +
		  SignedByte (branchTarget);
	      break;
	    }
	  break;
	case 1:
	  switch (GetField (Field.q, b))
	    ...
	    case 0:
	      ...
	      CurrentInstruction.InstructionArguments =
		TableRP[GetField (Field.p, b)] + ", " +
		ReadImmWord ().ToString (WordDataFormatString);
	      break;
	    case 1:
	      CurrentInstruction.InstructionName = "add";
	      CurrentInstruction.InstructionArguments =
		"hl, " + TableRP[GetField (Field.p, b)];
	      break;
	    }
	  break;
	case 2:
	  switch (GetField (Field.y, b))
		...
		ReadImmWord ().ToString (WordDataFormatString) + "), hl";
	      break;
	    case 5:
	      CurrentInstruction.InstructionName = "ld";
	      CurrentInstruction.InstructionArguments =
		"hl, (" + LocationPrefixString +
		ReadImmWord ().ToString (WordDataFormatString) + ")";
	      break;
	    case 6:
	      CurrentInstruction.InstructionName = "ld";
	      CurrentInstruction.InstructionArguments =
		"(" + LocationPrefixString +
		ReadImmWord ().ToString (WordDataFormatString) + "), a";
	      break;
	    case 7:
	      CurrentInstruction.InstructionName = "ld";
	      CurrentInstruction.InstructionArguments =
		"a, (" + LocationPrefixString +
		ReadImmWord ().ToString (WordDataFormatString) + ")";
	      break;
	    }
	  break;
...
// So unwind CurrentByte so we can process it again.
      ...
      else
	{
	  ...
	  CurrentInstruction.InstructionArguments =
	    TableR[GetField (Field.y, b)] + ", " +
	    TableR[GetField (Field.z, b)];
	}
      break;
    case 2:
      CurrentInstruction.InstructionName = TableAlu[GetField (Field.y, b)];
      CurrentInstruction.InstructionArguments =
	TableAluArg[GetField (Field.y, b)] + TableR[GetField (Field.z, b)];
      break;
    case 3:
      switch (GetField (Field.z, b))
	{
	...
	case 2:
	  ...
	  CurrentInstruction.InstructionArguments =
	    TableCC[GetField (Field.y, b)] + ", " + (branchTarget -
						     BaseAddress >= 0
						     && branchTarget -
						     BaseAddress <
						     Data.
	...
	case 3:
	  switch (GetField (Field.y, b))
	    ...
	    case 0:
	      ...
	      branchTarget = ReadImmWord ();
	      CurrentInstruction.InstructionArguments =
		(branchTarget - BaseAddress >= 0
		 && branchTarget - BaseAddress <
		 Data.Length ? LabelPrefixString : LabelPrefixString) +
		branchTarget.ToString (WordDataFormatString);
	      CurrentInstruction.IsBranch = true;
	      CurrentInstruction.BranchTarget = branchTarget;
	      break;
	    ...
	    case 2:
	      ...
	      CurrentInstruction.InstructionArguments =
		"(" + Data[CurrentByte++].ToString ("X2") + "), a";
	      break;
	    case 3:
	      CurrentInstruction.InstructionName = "in";
	      CurrentInstruction.InstructionArguments =
		"a, (" + Data[CurrentByte++].ToString ("X2") + ")";
	      break;
	    case 4:
	      CurrentInstruction.InstructionName = "ex";
	      CurrentInstruction.InstructionArguments = "(sp), hl";
	...
	case 6:
	  CurrentInstruction.InstructionName =
	    TableAlu[GetField (Field.y, b)];
	  CurrentInstruction.InstructionArguments =
	    TableAluArg[GetField (Field.y, b)] +
	    Data[CurrentByte++].ToString ("X2");
	  break;
	case 7:
	  CurrentInstruction.InstructionName = "rst";
	  CurrentInstruction.InstructionArguments =
	    (GetField (Field.y, b) * 8).ToString ("X2") + "h";
	  break;
	}
      break;
    }
...
DoCBPrefix ()
{
  ...
  switch (GetField (Field.x, b))
    ...
    case 1:
      ...
      CurrentInstruction.InstructionArguments =
	GetField (Field.y,
		  b).ToString () + ", " + TableR[GetField (Field.z, b)];
      break;
    case 2:
  ...
  switch (GetField (Field.x, b))
	...
	case 3:
	  ...
	  if (GetField (Field.q, b) == 0)
		...
		case 3:
	      ...
	      CurrentInstruction.InstructionArguments =
		tempstr + ", " + indexreg + " + " +
		((sbyte) Data[CurrentByte++]).ToString ("X2");
	    }
	  else
	    CurrentInstruction.InstructionName = "OPCODETRAP";
	  break;
	...
	case 3:
	  ...
	  if (GetField (Field.q, b) == 0)
	    ...
	    CurrentInstruction.InstructionArguments =
	      TableRP[GetField (Field.p, b)] + ", (" + LocationPrefixString +
	      ReadImmWord ().ToString (WordDataFormatString) + ")";
	  break;
	case 4:
	  if (Z80PlainMode)
		...
		switch (GetField (Field.p, b))
		  ...
		  case 1:
		    CurrentInstruction.InstructionName = "lea";
		    CurrentInstruction.InstructionArguments =
		      "ix, iy + " + SignedByte ((sbyte) Data[CurrentByte++]);
		    break;
		  case 2:
		    CurrentInstruction.InstructionName = "tst";
		    CurrentInstruction.InstructionArguments =
	...
	case 5:
	  if (Z80PlainMode)
		...
		case 2:
		  CurrentInstruction.InstructionName = "lea";
		  CurrentInstruction.InstructionArguments =
		    "iy, ix + " + SignedByte ((sbyte) Data[CurrentByte++]);
		  break;
		case 3:
		case 6:
		  CurrentInstruction.InstructionName = "OPCODETRAP";
		...
		case 4:
		  ...
		  CurrentInstruction.InstructionArguments =
		    "ix + " + SignedByte ((sbyte) Data[CurrentByte++]);
		  break;
		case 5:
		  CurrentInstruction.InstructionName = "ld";
	...
	case 6:
	  if (Z80PlainMode)
		...
		case 4:
		  CurrentInstruction.InstructionName = "pea";
		  CurrentInstruction.InstructionArguments =
		    "iy + " + SignedByte ((sbyte) Data[CurrentByte++]);
		  break;
		case 5:
		  CurrentInstruction.InstructionName = "ld";
...
namespace eZDisasm
{
  public class eZ80Disassembler
  {
    ...
    public
      public static DisassembledInstruction[] Disassemble (byte[]data,
							   int baseAddress =
							   0,
							   bool hasBaseAddress
							   =
							   ...
							   "")
    {
      List < DisassembledInstruction > disasm =
	...
	CurrentInstruction.Length =
	  (byte) (CurrentByte - CurrentInstruction.StartPosition);
      }
	...
	CurrentInstruction.InstructionSuffix = "";
	CurrentInstruction.Length =
	  (byte) (Data.Length - CurrentInstruction.StartPosition);
      }
	...
	switch (b)
	  ...
	  case 0x34:
	    ...
	    CurrentInstruction.InstructionArguments =
	      "(" + indexRegister + " + " +
	      SignedByte ((sbyte) Data[CurrentByte++]) + ")";
	    break;
	  case 0x25:
	    CurrentInstruction.InstructionName = "dec";
	    CurrentInstruction.InstructionArguments = indexRegister + "h";
	  ...
	  case 0x6D:
	    CurrentInstruction.InstructionName = "ld";
	    CurrentInstruction.InstructionArguments =
	      indexRegister + TableR[GetField (Field.y, b)] + ", " +
	      indexRegister + TableR[GetField (Field.z, b)];
	    break;
	  case 0x70:
	  case 0x71:
	  case 0x72:
	  ...
	  case 0x7E:
	    CurrentInstruction.InstructionName = "ld";
	    CurrentInstruction.InstructionArguments =
	      TableR[GetField (Field.y, b)] + ", (" + indexRegister + " + " +
	      SignedByte ((sbyte) Data[CurrentByte++]) + ")";
	    break;
	  case 0x84:
	  case 0x85:
	  ...
	  case 0xBD:
	    CurrentInstruction.InstructionName =
	      TableAlu[GetField (Field.y, b)];
	    CurrentInstruction.InstructionArguments =
	      TableAluArg[GetField (Field.y, b)] + indexRegister +
	      TableR[GetField (Field.z, b)];
	    break;
	  case 0x86:
	  case 0x8E:
	  ...
	  case 0xCB:
	    ...
	    switch (GetField (Field.x, b))
	      ...
	      case 0:
		CurrentInstruction.InstructionName =
		  TableRot[GetField (Field.y, b)];
		if (GetField (Field.z, b) != 6)
		  if (Z80PlainMode)
		    CurrentInstruction.InstructionArguments =
		      TableR[GetField (Field.z, b)] + ", (" + indexRegister +
		      " + " + SignedByte ((sbyte) Data[CurrentByte - 1]) +
		      ")";
		  else
		    CurrentInstruction.InstructionName = "OPCODETRAP";
		else
		  CurrentInstruction.InstructionArguments =
		    TableRot[GetField (Field.y, b)] + "(" + indexRegister +
		    " + " + SignedByte ((sbyte) Data[CurrentByte - 1]) + ")";
		break;
	      case 1:
		CurrentInstruction.InstructionName = "bit";
	  ...
	  case 0x27:
	    ...
	    CurrentInstruction.InstructionName = "ld";
	    CurrentInstruction.InstructionArguments =
	      TableRP[GetField (Field.p, b)] + ", (" + indexRegister + " + " +
	      SignedByte ((sbyte) Data[CurrentByte++]) + ")";
	    break;
	  case 0x37:
	    if (Z80PlainMode)
	  ...
	  case 0x31:
	    ...
	    CurrentInstruction.InstructionName = "ld";
	    CurrentInstruction.InstructionArguments =
	      TableIndex[indexRegNumber ^ 1] + ", (" + indexRegister + " + " +
	      SignedByte ((sbyte) Data[CurrentByte++]) + ")";
	    break;
	  case 0x3E:
	    if (Z80PlainMode)

...
const char *dname[10] = {"drivea.cpm","driveb.cpm","drivec.cpm","drived.cpm"
		,"","","","","drivei.cpm","drivej.cpm"};
...
public:
	...
	,_tracks(((RDSK_SecSize << dpb.bsh) * (dpb.dsm + 2) - RDSK_SecSize) / RDSK_SecSize / dpb.spt + dpb.off)
	{
	...
	bool Read(uint16_t track, uint8_t sect, uint8_t* dma)
	{
		...
		off_t off = (track * _dpb.spt + _xlt[sect-1]-1) * RDSK_SecSize;
		clog << "Read trk " << track << ", sec " << (unsigned)sect << ", dma " << (void*)dma << endl;
...
int main(int argc, char *argv[]) {
	...
	while (1)
	{
		...
		if(RDSK_CmdMount == pkt.hdr.RDSK_Code)
		{
			...
			if(id < 10 && (Session[id] ||
			   (Session[id] = new Drive (dname[id], pkt.req_mount.RDSK_Dpb/*&req.RDSK_Map*/))) &&
				Session[id]->Mount())
			{
